            ...
            ForEach(Dispute.DisputeReason.allCases) { reason in
                Button {
                    ...
                    HStack(spacing: 14) {
                        ZStack {
                            Circle()
                                .fill(selectedReason == reason ? Color.black : Color(sxHex: "#efefef"))
                                .frame(width: 36, height: 36)
                            Image(systemName: reason.icon)
                                .font(.system(size: 15))
                                .foregroundStyle(selectedReason == reason ? .white : Color(sxHex: "#666666"))
                        }
                        Text(reason.label)
    ...
    private var submitButton: some View {
        Button {
            ...
            .background(description.trimmingCharacters(in: .whitespacesAndNewlines).count >= 10
                        ? Color.black : Color(sxHex: "#cccccc"))
            .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))
        }
...
struct DisputeStatusBadge: View {
    ...
    private var color: Color {
        ...
        case .submitted, .reviewing: return Color(sxHex: "#f59e0b")
        case .resolved, .refunded:   return Color(sxHex: "#22c55e")
        case .rejected:              return Color(sxHex: "#888888")
        }
    }
